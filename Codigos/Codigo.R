...
CY08MSP_STU_QQQ <- read_sas("CY08MSP_STU_QQQ.SAS7BDAT", 
                            +     NULL)

CY08MSP_SCH_QQQ <- read_sas("CY08MSP_SCH_QQQ.SAS7BDAT", 
                            +     NULL)

CY08MSP_TCH_QQQ <- read_sas("CY08MSP_TCH_QQQ.SAS7BDAT", 
                            +     NULL)
...
school <- CY08MSP_SCH_QQQ %>%
  select(CNT, CNTSCHID, SC013Q01TA, SC016Q01TA, SC004Q02TA, SC004Q03TA, SC004Q07NA, STRATIO, 
...
# Juntar las tablas por ID de escuela y ID de pais ----

escuela_stu <- students %>%
  left_join(school, by = c("CNT", "CNTSCHID"))


pisa_2022 <- escuela_stu %>%
  left_join(teacher, by = c("CNT", "CNTSCHID"), relationship = "many-to-many")
